...
function inferCityFromLocation(location) {
  ...
  if (!normalizedLocation || normalizedLocation === "Unknown Location") {
    return "Unknown City";
  }

  const commaParts = normalizedLocation
    .split(",")
    .map((part) => cleanText(part))
    .filter(Boolean);

  if (commaParts.length >= 2 && isValidLocationPart(commaParts[1])) {
    return commaParts[1];
  }

  const words = normalizedLocation.split(/\s+/).filter(Boolean);
