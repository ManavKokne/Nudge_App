...
export async function insertAlertFromPost({
  ...
  const columnValuePairs = [
    ...
    [columnMap.requestTypeColumn, normalizedRequestType],
    [columnMap.urgencyColumn, dashboardUrgency],
  ];

  if (columnMap.cityColumn) {
    columnValuePairs.push([columnMap.cityColumn, normalizedCity]);
  }
  ...
  if (columnMap.geocodeStatusColumn) {
    columnValuePairs.push([columnMap.geocodeStatusColumn, "pending"]);

    if (columnMap.latitudeColumn) {
      columnValuePairs.push([columnMap.latitudeColumn, null]);
    }

    if (columnMap.longitudeColumn) {
      columnValuePairs.push([columnMap.longitudeColumn, null]);
    }
  }
...
export async function updateAlertBySourcePostId({
  ...
  if (columnMap.cityColumn) {
    setPairs.push([columnMap.cityColumn, normalizedCity]);
  }
  ...
  if (columnMap.geocodeStatusColumn) {
    setPairs.push([columnMap.geocodeStatusColumn, "pending"]);

    if (columnMap.latitudeColumn) {
      setPairs.push([columnMap.latitudeColumn, null]);
    }

    if (columnMap.longitudeColumn) {
      setPairs.push([columnMap.longitudeColumn, null]);
    }
  }
...
export async function getAlertClusterBySourcePostId(sourcePostId, { client, forUpdate = false } = {}) {
  const columns = await getDisasterTweetColumns(client);
  const columnMap = getTweetColumnMap(columns);
  ...
  const rowLockClause = forUpdate ? " FOR UPDATE" : "";
...
export async function deactivateAlertBySourcePostId(sourcePostId, { client } = {}) {
  ...
  if (columnMap.isClosedColumn || columnMap.isInformativeColumn) {
    ...
    if (columnMap.geocodeStatusColumn) {
      values.push("pending");
      setClauses.push(`${quoteIdentifier(columnMap.geocodeStatusColumn)} = $${values.length}`);

      if (columnMap.latitudeColumn) {
        values.push(null);
        setClauses.push(`${quoteIdentifier(columnMap.latitudeColumn)} = $${values.length}`);
      }

      if (columnMap.longitudeColumn) {
        values.push(null);
        setClauses.push(`${quoteIdentifier(columnMap.longitudeColumn)} = $${values.length}`);
      }
    }
