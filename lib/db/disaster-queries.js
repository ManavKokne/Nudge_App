...
export async function countSimilarAlerts({ city, requestType, excludeSourcePostId }) {
  const columns = await getDisasterTweetColumns();
  const cityColumn = pickColumn(columns, ["city"]) || pickColumn(columns, ["location"]);
  ...
  if (!cityColumn || !requestTypeColumn || !createdAtColumn) {
    throw new Error(
  ...
  const queryText = `SELECT COUNT(*)::int AS count
                     FROM public.tweets
                     WHERE ${quoteIdentifier(cityColumn)} = $1
...
export async function insertAlertFromPost({
  ...
  if (cityColumn) {
    columnValuePairs.push([cityColumn, city]);
  }
