...
export async function recomputeSocialClusterUrgency({ city, requestType, urgencyScore, urgencyLabel }) {
  const normalizedCity = String(city || "").trim().toLowerCase();
  const normalizedRequestType = String(requestType || "").trim().toLowerCase();

  if (!normalizedCity || !normalizedRequestType || urgencyScore == null || !urgencyLabel) {
    return {
      city: normalizedCity || null,
      requestType: normalizedRequestType || null,
      updatedCount: 0,
    };
  }
  ...
  const result = await socialQuery(
    `UPDATE posts
     SET urgency_score = $3,
         urgency_label = $4,
         updated_at = NOW()
     WHERE LOWER(COALESCE(extracted_city, '')::text) = $1
       AND LOWER(COALESCE(extracted_request_type, '')::text) = $2
       AND created_at > NOW() - INTERVAL '1 hour'
       AND COALESCE(extracted_is_informative, FALSE) = TRUE
       AND COALESCE(content, '') NOT ILIKE 'SOS Alert:%'`,
    [normalizedCity, normalizedRequestType, urgencyScore, urgencyLabel]
  );
...
export async function syncSocialUrgencyForRecomputedClusters(recomputedClusters = []) {
  const updates = [];

  for (const cluster of recomputedClusters) {
    if (!cluster?.city || !cluster?.requestType || cluster.urgencyScore == null || !cluster.urgencyLabel) {
      continue;
    }

    updates.push(
      recomputeSocialClusterUrgency({
        city: cluster.city,
        requestType: cluster.requestType,
        urgencyScore: cluster.urgencyScore,
        urgencyLabel: cluster.urgencyLabel,
      })
    );
  }
